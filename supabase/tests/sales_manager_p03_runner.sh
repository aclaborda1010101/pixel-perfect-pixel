...
# smp03_test_<random> y un ROL DEDICADO no-superusuario. TODAS las
...
if [ "$(id -u)" = "0" ]; then
  RUNAS="${SMP03_LOCAL_USER:-}"
  if [ -z "$RUNAS" ] || ! id "$RUNAS" >/dev/null 2>&1 || ! command -v setpriv >/dev/null 2>&1; then
    skip "PostgreSQL no arranca como root. Define SMP03_LOCAL_USER con un usuario local sin privilegios."
  fi
  exec setpriv --reuid="$(id -u "$RUNAS")" --regid="$(id -g "$RUNAS")" --clear-groups \
       env HOME=/tmp SMP03_LOCAL_USER= bash "${BASH_SOURCE[0]}" "$@"
fi

RAND="$(od -An -N4 -tu4 /dev/urandom | tr -d ' ')"
TESTDB="smp03_test_${RAND}"
ROLE="smp03_role_${RAND}"
ADMIN="smp03_admin_${RAND}"
case "$TESTDB" in smp03_test_*) ;; *) die "nombre de base inválido: $TESTDB";; esac

WORK="$(mktemp -d "${TMPDIR:-/tmp}/smp03.XXXXXXXX")" || skip "no se puede crear directorio temporal"
DATA="$WORK/data"
SOCK="$WORK/sock"
mkdir -p "$SOCK"
...
# --- 4. El cierre de has_role debe haberse consumado ------------------
"${PSQL_TEST[@]}" -c "DO \$\$
BEGIN
  IF to_regprocedure('public.current_user_has_role(public.app_role)') IS NULL
     OR to_regprocedure('public.internal_member_has_role(uuid, public.app_role)') IS NULL
     OR to_regprocedure('public.get_sales_manager_dashboard(timestamptz, timestamptz)') IS NULL THEN
    RAISE EXCEPTION 'la cadena sales_manager no está completa en la base de prueba';
  END IF;
  IF has_function_privilege('authenticated','public.has_role(uuid, public.app_role)','EXECUTE') THEN
    RAISE EXCEPTION 'has_role sigue ejecutable por authenticated tras la migración de cierre';
  END IF;
  IF EXISTS (SELECT 1 FROM pg_policies
             WHERE (COALESCE(qual,'')||COALESCE(with_check,'')) ~ '[^_]has_role\\(') THEN
    RAISE EXCEPTION 'quedan políticas sin portar a current_user_has_role';
  END IF;
END \$\$;" || die "verificación de cierre de has_role fallida"
...
for f in "$ROOT/supabase/tests/sales_manager_p03_rls.sql" \
         "$ROOT/supabase/tests/sales_manager_p03_metrics.sql"; do
  [ -f "$f" ] || continue
...
run_suite "rls_roles_p03" "$ROOT/supabase/tests/sales_manager_p03_rls.sql"
run_suite "metricas_p03"  "$ROOT/supabase/tests/sales_manager_p03_metrics.sql"

if [ "$FAILED" != "0" ]; then
  echo "SALES_MANAGER P0.3 · integración: NO-GO (al menos un caso en rojo)"
  exit 1
fi
echo "SALES_MANAGER P0.3 · integración: PASS (clúster efímero, base $TESTDB, rol $ROLE)"
