#!/usr/bin/env bash
# =====================================================================
# MOTOR V5 P0.3 · RUNNER LOCAL DESECHABLE (IMPOSIBLE CONTRA PRODUCCIÓN)
# =====================================================================
...
# v5p03_test_<random> y un ROL DEDICADO no-superusuario. TODAS las
...
if [ "$(id -u)" = "0" ]; then
  RUNAS="${V5P03_LOCAL_USER:-}"
  if [ -z "$RUNAS" ] || ! id "$RUNAS" >/dev/null 2>&1 || ! command -v setpriv >/dev/null 2>&1; then
    skip "PostgreSQL no arranca como root. Define V5P03_LOCAL_USER con un usuario local sin privilegios."
  fi
  exec setpriv --reuid="$(id -u "$RUNAS")" --regid="$(id -g "$RUNAS")" --clear-groups \
       env HOME=/tmp V5P03_LOCAL_USER= bash "${BASH_SOURCE[0]}" "$@"
fi

RAND="$(od -An -N4 -tu4 /dev/urandom | tr -d ' ')"
TESTDB="v5p03_test_${RAND}"
ROLE="v5p03_role_${RAND}"
ADMIN="v5p03_admin_${RAND}"
case "$TESTDB" in v5p03_test_*) ;; *) die "nombre de base inválido: $TESTDB";; esac

WORK="$(mktemp -d "${TMPDIR:-/tmp}/v5p03.XXXXXXXX")" || skip "no se puede crear directorio temporal"
DATA="$WORK/data"
SOCK="$WORK/sock"
mkdir -p "$SOCK"
...
CHAIN+=("$ROOT/supabase/pending_migrations/20260811000000_sales_manager_phase_b.sql")
CHAIN+=("$ROOT/supabase/pending_migrations/20260813120000_sales_manager_p03_has_role_lockdown.sql")
# Cadena V5 COMPLETA: Fase A -> P0.2 -> P0.3, aplicada DESPUÉS de sembrar
# tareas con clave histórica v5:<fecha>:T-0X reales.
CHAIN+=("$ROOT/supabase/pending_migrations/20260811230000_v5_engine_phase_a.sql")
CHAIN+=("$ROOT/supabase/pending_migrations/20260813000000_v5_engine_p02_runtime.sql")
CHAIN+=("$ROOT/supabase/pending_migrations/20260815000000_v5_engine_p03_runtime_connect.sql")
...
for f in "${CHAIN[@]}"; do
  ...
  asegurar_placeholder
  if [ "$base" = "20260811230000_v5_engine_phase_a.sql" ]; then
    # Fixture HISTÓRICO REAL: la clave antigua debe sobrevivir a la cadena.
    "${PSQL_TEST[@]}" -c "INSERT INTO public.building_tasks
        (id, building_id, title, priority, status, task_type, task_key)
      VALUES (gen_random_uuid(), '0485d8cf-c1a2-4412-b38f-e37fb18961a2',
              'Primera llamada (histórico)', 'high', 'pending', 'call_queue',
              'v5:2026-08-10:T-02:11111111-1111-1111-1111-111111111111')
      ON CONFLICT DO NOTHING;" >/dev/null 2>&1 \
      || die "no se pudo sembrar el fixture histórico previo a la Fase A"
  fi
...
# --- 4. Verificación por DEFINICIÓN (no sólo existencia) --------------
"${PSQL_TEST[@]}" -c "DO \$\$
DECLARE v_def text;
BEGIN
  IF to_regprocedure('public.commit_v5_generation_plan(uuid, uuid, jsonb)') IS NULL
     OR to_regprocedure('public.resolve_building_task(uuid, text, text)') IS NULL
     OR to_regprocedure('public.claim_v5_generation_requests(int, uuid[])') IS NULL
     OR to_regprocedure('public.reap_v5_generation_leases()') IS NULL THEN
    RAISE EXCEPTION 'la cadena V5 P0.3 no está completa';
  END IF;

  SELECT pg_get_constraintdef(oid) INTO v_def FROM pg_constraint
   WHERE conrelid = 'public.building_tasks'::regclass
     AND conname = 'building_tasks_v5_canonical_key_chk';
  IF v_def IS NULL OR v_def NOT LIKE '%production%' OR v_def NOT LIKE '%T2_T3%' THEN
    RAISE EXCEPTION 'clave canónica: definición inesperada (%)', coalesce(v_def, 'NULL');
  END IF;

  SELECT pg_get_indexdef(indexrelid) INTO v_def FROM pg_index
   WHERE indexrelid = 'public.building_tasks_one_active_production_uidx'::regclass;
  IF v_def IS NULL OR v_def NOT LIKE '%pending%' OR v_def LIKE '%blocked%' THEN
    RAISE EXCEPTION 'predicado de slot inesperado (%)', coalesce(v_def, 'NULL');
  END IF;

  -- El histórico sobrevive, etiquetado legacy y sin ocupar slot.
  IF NOT EXISTS (SELECT 1 FROM public.building_tasks
                  WHERE task_key = 'v5:2026-08-10:T-02:11111111-1111-1111-1111-111111111111'
                    AND generation_mode = 'legacy') THEN
    RAISE EXCEPTION 'el fixture histórico no sobrevivió como legacy';
  END IF;

  IF has_function_privilege('authenticated','public.commit_v5_generation_plan(uuid, uuid, jsonb)','EXECUTE') THEN
    RAISE EXCEPTION 'authenticated no puede ejecutar el commit del motor';
  END IF;
END \$\$;" || die "verificación estructural V5 P0.3 fallida"
...
for f in "$ROOT/supabase/tests/v5_p03_cases.sql"; do
  [ -f "$f" ] || continue
...
run_suite "runtime_v5_p03" "$ROOT/supabase/tests/v5_p03_cases.sql"

if [ "$FAILED" != "0" ]; then
  echo "MOTOR V5 P0.3 · integración: NO-GO (al menos un caso en rojo)"
  exit 1
fi
echo "MOTOR V5 P0.3 · integración: PASS (clúster efímero, base $TESTDB, rol $ROLE)"
