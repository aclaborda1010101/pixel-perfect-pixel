#!/usr/bin/env bash
# REPARSEO P0.11 — runner AISLADO (PostgreSQL efímero, sin red).
#  - Aplica la cadena REAL de migraciones pendientes: P0.5 -> P0.6 -> P0.8 -> P0.11.
...
TMP="$(mktemp -d /tmp/p11cluster.XXXXXX)"
...
$AS initdb -D "$DATA" -U p11owner --auth=trust >"$TMP/initdb.log" 2>&1
$AS pg_ctl -D "$DATA" -o "-k $SOCK -c listen_addresses=''" -l "$TMP/pg.log" start >/dev/null
export PGHOST="$SOCK" PGUSER=p11owner PGDATABASE=postgres
psql -v ON_ERROR_STOP=1 -q -c "CREATE DATABASE p11;"
export PGDATABASE=p11
...
echo "== esquema + cadena REAL de migraciones (P0.5, P0.6, P0.8, P0.11) =="
MIGS=(
  ...
  supabase/pending_migrations/20260818000000_reparse_p11_reemplazo_registral.sql
...
P11_LIVE=1 P11_PGHOST="$SOCK" P11_PGDATABASE=p11 P11_PGUSER=service_role P11_PGOWNER=p11owner \
  bunx vitest run --fileParallelism=false src/test/reparseP11Live.test.ts

echo "PASS · P0.11 verificado contra PostgreSQL efímero real"
