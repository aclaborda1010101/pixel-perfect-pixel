// recount-windows-cal9 — anti-sobreestimacion (miradores=1 eje, no doble cuenta esquinas, fila_mas_limpia)
// Clon de cal8 con 3 fixes; CHUNKED + chained re-invocation, 1 edificio/lote, <=150s.
...
const KEY_OUT = "recount_windows_cal9_last";
...
const PROMPT = (plantas: number, oclusionPrev: string | null) => `Arquitecto tecnico. Capturas Street View de la MISMA fachada residencial de Madrid. Plantas sobre rasante (Catastro): ${plantas}.

TAREA: cuenta EJES VERTICALES de huecos (columnas continuas) en ESTA fachada.

REGLAS ESTRICTAS ANTI-SOBREESTIMACION:
- MIRADOR / GALERIA / TRIBUNA = SIEMPRE 1 EJE, aunque tenga 2-3-4 panos de vidrio. NUNCA cuentes cada pano del mirador como eje separado. Reporta el numero de miradores aparte.
- BALCONERA-VENTANA cuenta como eje; BALCONERA-PUERTA o puerta de portal NO cuentan.
- PB con escaparates: no son ejes residenciales; van en huecos_planta_baja.
- ESQUINA / CHAFLAN: si ves el mismo eje desde dos angulos NO lo cuentes dos veces. Solo cuenta ejes que pertenecen a ESTA fachada (no a la perpendicular). Reporta ejes_chaflan aparte (eje justo en la esquina, compartido entre dos fachadas: vale 0.5 en cada una).
- RETRANQUEO / doble cuerpo: suma ejes principales + retranqueo (cuerpos_independientes=true) PERO no dupliques el eje compartido.
- Si fachada muy ocluida -> oclusion_alta=true y dime fila_mas_limpia (planta + nº huecos). En ese caso ejes_verticales = huecos de fila_mas_limpia (es el dato fiable).
- Si <2 plantas claras -> confianza="baja".
- Prohibido inventar.

${oclusionPrev ? `(Capturas previas: ${oclusionPrev})\n` : ""}
JSON:
{"huecos_por_planta":[number,...],"ejes_verticales":number,"huecos_planta_baja":number,
 "miradores_detectados":number,"ejes_chaflan":number,
 "oclusion_alta":boolean,"fila_mas_limpia":{"planta":string,"huecos":number}|null,
...
async function processOne(sb: any, apiKey: string, gKey: string, bid: string) {
  ...
  const isCorner = Object.keys(byRole).filter(r => r !== "patio").length >= 2;
  ...
  for (const [role, ps] of Object.entries(byRole)) {
    ...
    const ejesArr: number[] = []; const pbArr: number[] = []; const chaflanArr: number[] = []; const detalle: any[] = [];
    ...
    for (const p of panosUse) {
      for (const off of offsets) {
        ...
        const urls: string[] = [];
        for (const h of headings) urls.push(sv(lat, lng, h, 15, 70, gKey));
        try {
          ...
          const filaH = Number(v?.fila_mas_limpia?.huecos);
          const chaflan = Number(v.ejes_chaflan ?? 0);
          // FIX 3: si hay fila_mas_limpia fiable, esa MANDA (anti-sobreestimacion por proyeccion)
          if (Number.isFinite(filaH) && filaH > 0) {
            if (v.oclusion_alta || filaH < ejes) {
              ejes = filaH;
              lastOclusion = `fila ${v?.fila_mas_limpia?.planta} con ${filaH} huecos`;
            }
          }
          if (ejes > 0) ejesArr.push(ejes);
          pbArr.push(pb);
          if (Number.isFinite(chaflan) && chaflan > 0) chaflanArr.push(chaflan);
          detalle.push({ role, off, ejes, pb, chaflan, miradores: v.miradores_detectados, ocl: v.oclusion_alta, fila: v.fila_mas_limpia, cuerpos: v.cuerpos_independientes, conf: v.confianza });
    ...
    const chaflan_med = chaflanArr.length ? median(chaflanArr) : 0;
    // FIX 2: chaflan se cuenta a medias entre fachadas; lo acumulamos aparte y lo añadimos UNA vez al final
    if (chaflan_med > 0) totalChaflan += chaflan_med * 0.5;
    const total = ejes_med * plantas_tipo + pb_med;
    facadeResults.push({ role, n_panos: ps.length, offsets, ejes_med, pb_med, chaflan_med, total, detalle });
  }

  const chaflanFinal = isCorner ? Math.round(totalChaflan * plantas_tipo) : 0;
  const total_pred = facadeResults.reduce((s, f) => s + f.total, 0) + chaflanFinal;
  ...
  return {
    ...
    is_corner: isCorner, chaflan_aporte: chaflanFinal,
    gt: gt_val, pred_anterior: fwc?.final_count, pred_cal9: total_pred,
...
async function reinvoke(payload: any) {
  try {
    await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/recount-windows-cal9`, {
      method: "POST",
  ...
  } catch (e) { console.warn("cal9 reinvoke fail", (e as Error).message); }
}
...
Deno.serve(async (req) => {
  ...
  EdgeRuntime.waitUntil((async () => {
    ...
    console.log("cal9 batch", { processed: batch.length, remaining: rest.length, ...status });
