...
Deno.serve(async(req)=>{
  ...
  const run=async()=>{
    const {data:prev}=await sb.from("app_settings").select("value").eq("key","recount_windows_cal6_last").maybeSingle();
    ...
    for(const bid of slice){
      try{
        ...
        for(const {role,edge} of roles){
          ...
          // Captura HALVES (siempre, también <50m)
          ...
          const halfEjes=halves.reduce((s,h)=>s+Number(h.ejes_completos??0),0);
          const halfPb=halves.reduce((s,h)=>s+Number(h.pb_completos??0),0);
          // Tomar MAX (cubre oclusión: halves suelen ver más). PB: max también.
          const ejes_role=Math.max(fullEjes, halfEjes);
          const pb_role=Math.max(fullPb, halfPb);
          const total_role=ejes_role*pt+pb_role;
          facadeR.push({role,len_m:len,fullEjes,halfEjes,fullPb,halfPb,ejes_role,pb_role,total_role,full:vF,halves});
        }
        const total=facadeR.reduce((s,f)=>s+f.total_role,0);
        const ape=gt?Math.abs(total-gt)/gt*100:null;
        const row={building_id:bid,direccion:b?.direccion,rc14,plantas,plantas_tipo:pt,is_corner,gt,pred_cal6:total,
        ...
        await sb.from("app_settings").upsert({key:"recount_windows_cal6_last",
    ...
    await sb.from("app_settings").upsert({key:"recount_windows_cal6_last",
      ...
      updated_at:new Date().toISOString()},{onConflict:"key"});
    console.log("cal6 batch done",JSON.stringify({mape,w10,w15,n:out.length,remaining:remaining.length}));
    if(chain && remaining.length){
      try{
        const srk=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
        await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/recount-windows-cal6`,{
      ...
      }catch(e){ console.warn("cal6 chain fail",(e as Error).message); }
    }
  };
