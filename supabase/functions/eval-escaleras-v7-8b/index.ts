// eval-escaleras-v7.8b — variante de v7.8-zoom con:
//  - umbral Pass B = 0.5 (en lugar de 0.7)
//  - OCR reconoce COM.V / COM.VA / COM.VB / COM.VC (notación catastral
//    de "Común vertical" = núcleo de escalera) además de ESCALERA/ESC.
//  Solo escribe en escaleras_eval_results version='v7.8b-zoom'.
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.45.0";
import { corsHeaders } from "npm:@supabase/supabase-js@2/cors";

const MODEL = "google/gemini-2.5-pro";
const VERSION = "v7.8b-zoom";
const MIN_CONF_B = 0.5;
...
3) OCR de leyenda/cuadros: lista TOKENS literales relevantes encontrados
   en la lámina: "ESCALERA","ESC","E1","E2","NÚCLEO",
   letras de escalera (A,B,C...), "COM.V","COM.VA","COM.VB","COM.VC"
   (común vertical = núcleo de escalera en notación catastral),
   "ASCENSOR","PATINILLO","V.A","V.B"...
...
¿Es una CAJA DE ESCALERA real (núcleo con peldaños/diagonales que sirve
a viviendas)? Núcleo escalera+ascensor cuenta SÍ. Patinillo o hueco de
ascensor solo NO. Si no se ve clara, baja confianza.
...
function countEscalerasFromTokens(tokens: string[]): number {
  const letters = new Set<string>();
  const comvLetters = new Set<string>();
  let comvBare = 0;
  let escaleraOcc = 0;
  for (const t of (tokens ?? [])) {
    const u = String(t).toUpperCase().trim();
    if (/^ESCALERA$/.test(u)) escaleraOcc++;
    let m;
    if ((m = u.match(/^ESC[\.\s]*([A-D])$/)) || (m = u.match(/^ESCALERA\s+([A-D])$/)) || (m = u.match(/^E\s*([1-4])$/))) {
      letters.add(m[1]);
    }
    if ((m = u.match(/^COM\.V([A-D])$/))) comvLetters.add(m[1]);
    else if (/^COM\.V$/.test(u)) comvBare++;
  }
  if (comvLetters.size > 0) return comvLetters.size;
  if (letters.size > 0) return letters.size;
  if (escaleraOcc > 0) return escaleraOcc;
  if (comvBare > 0) return 1; // un único núcleo común vertical visible
  return 0;
}
...
async function evalOne(sb: any, apiKey: string, set_name: string, bid: string) {
  ...
  const best = p01s[0];
  if (!best) {
    return { pred_n: base.pred_n ?? null, needs_review: base.pred_n == null, confidence: 0, evidencia: { base, motivo: "sin P01 legible", passA } };
  }
  ...
  const confirmadas = passB.filter(b => b.es_caja === true && !b.solo_ascensor && (b.confidence ?? 0) >= MIN_CONF_B);
  ...
  const nOcr = Math.max(nLeyenda, countEscalerasFromTokens(tokensAll));
  let pred: number | null = null, needsReview = true, razon = "", conf = 0;
  if (nVisual > 0 && nOcr > 0 && nVisual === nOcr) { pred = nVisual; needsReview = false; conf = 0.9; razon = `visual==ocr==${nVisual}`; }
  else if (nVisual > 0 && nOcr === 0) { pred = nVisual; needsReview = false; conf = 0.7; razon = `solo visual=${nVisual}`; }
  else if (nOcr > 0 && nVisual === 0) { pred = nOcr; needsReview = false; conf = 0.65; razon = `solo ocr=${nOcr}`; }
  else if (nVisual > 0 && nOcr > 0 && nVisual !== nOcr) { pred = null; needsReview = true; conf = 0.4; razon = `discrepan v=${nVisual} o=${nOcr}`; }
  else { razon = "sin señales"; }
  return { pred_n: pred, needs_review: needsReview, confidence: conf, evidencia: { base, best_idx: best.idx, nVisual, nOcr, razon, passA, passB } };
}
...
Deno.serve(async (req) => {
  ...
  const run = async () => {
    for (const it of batch) {
      ...
      } catch (e) { console.warn("v7.8b err", it.building_id, (e as Error).message); }
      await new Promise(r => setTimeout(r, 300));
    }
    if (remaining.length) {
      try {
        await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/eval-escaleras-v7-8b`, {
          method: "POST",
      ...
      } catch (e) { console.warn("v7.8b reinvoke fail", (e as Error).message); }
    }
  };
