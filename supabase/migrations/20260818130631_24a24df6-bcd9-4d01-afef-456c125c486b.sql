-- 1) Aprobar ya NO marca 'aplicada': deja la fila en espera de escritura real.
...
BEGIN
  ...
  RETURN jsonb_build_object('accion','aprobar','aplicadas',0,
...
-- 2) Regla dura: 'aplicada' sólo la puede poner el proceso automático y con fecha.
...
DROP TRIGGER IF EXISTS trg_guard_proposals_aplicada ON public.guard_proposals;
CREATE TRIGGER trg_guard_proposals_aplicada
  BEFORE INSERT OR UPDATE ON public.guard_proposals
  FOR EACH ROW EXECUTE FUNCTION public.guard_proposals_solo_aplicada_con_escritura();