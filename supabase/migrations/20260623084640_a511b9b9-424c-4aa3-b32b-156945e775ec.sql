...
DO $$ BEGIN
  CREATE TYPE public.iee_estado AS ENUM (
    'desconocido','no_procede','pendiente','favorable','caducada',
    'desfavorable_leve','desfavorable_grave'
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 2. Columnas IEE en buildings
ALTER TABLE public.buildings
  ADD COLUMN IF NOT EXISTS iee_estado public.iee_estado NOT NULL DEFAULT 'desconocido',
  ADD COLUMN IF NOT EXISTS iee_fecha_inspeccion date,
  ADD COLUMN IF NOT EXISTS iee_proxima_revision date,
  ADD COLUMN IF NOT EXISTS iee_deficiencias jsonb DEFAULT '[]'::jsonb,
  ADD COLUMN IF NOT EXISTS iee_fuente text,
  ADD COLUMN IF NOT EXISTS iee_actualizado_at timestamptz;

-- 3. Trigger: mantener iee_proxima_revision y auto-flip a caducada
CREATE OR REPLACE FUNCTION public.tg_iee_maintain()
...
END;
$$;
DROP TRIGGER IF EXISTS buildings_iee_maintain ON public.buildings;
CREATE TRIGGER buildings_iee_maintain BEFORE INSERT OR UPDATE OF iee_estado, iee_fecha_inspeccion
  ON public.buildings FOR EACH ROW EXECUTE FUNCTION public.tg_iee_maintain();

-- 4. Helper: componentes IEE para el scoring
CREATE OR REPLACE FUNCTION public.iee_score_components(p_building_id uuid)
RETURNS TABLE(delta numeric, aviso jsonb, label text, estado text)
...
END;
$$;
GRANT EXECUTE ON FUNCTION public.iee_score_components(uuid) TO authenticated, service_role;

-- 5. Reemplazar compute_cluster_score con bloque IEE
...
BEGIN
  ...
  ELSIF v_protegido AND v_2esc_posible THEN
    v_avisos := v_avisos || jsonb_build_object('key','cambio_uso_sugerido_revisar','label','Posible cambio de uso — revisar 2a escalera','severity','medium',
      'detail','El analisis sugiere >=2 escaleras (Visor: ' || COALESCE(ba.n_escaleras_visor::text,'-') || ', conf ' || COALESCE(ba.escaleras_visor_confianza::text,'-') || '), pero NO esta confirmado por revision humana. No puntua hasta confirmar.');
  END IF;
  ...
  v_score := round(s_tamano*w_tamano + s_ratio*w_ratio + s_viv*w_viv + s_own*w_own + s_mg*w_mg + s_local*w_local + v_bonus + v_iee_delta - v_pen, 1);
