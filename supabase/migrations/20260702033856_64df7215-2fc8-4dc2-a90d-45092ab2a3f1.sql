...
-- 1) Nuevas columnas de auditoría en buildings
ALTER TABLE public.buildings
  ADD COLUMN IF NOT EXISTS grupo_barrio text,
  ADD COLUMN IF NOT EXISTS pct_terciario numeric,
  ADD COLUMN IF NOT EXISTS es_estrella boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS idx_buildings_es_estrella ON public.buildings (es_estrella) WHERE es_estrella = true;
CREATE INDEX IF NOT EXISTS idx_buildings_grupo_barrio ON public.buildings (grupo_barrio);

-- 2) compute_cluster_score: nueva lógica de alarmas + estrella + persistencia grupo/pct_terciario/es_estrella
...
BEGIN
  ...
  -- GRUPO DE BARRIO (9 prime exactos)
  ...
  -- ============ % TERCIARIO ============
  -- Primary: metadatos HubSpot. Fallback: dnprc_json (usos catastrales).
  ...
  -- Suelo mínimo si hay locales PB confirmados
  ...
  -- ⭐ ESTRELLA: terciario ≥66% + (2esc confirmada O n_escaleras_final ≥ 2). NO requiere protegido.
  IF COALESCE(v_terciario_pct,0) >= 0.66 AND (v_2esc_confirmada OR v_n_escaleras_final >= 2) THEN
    v_es_estrella := true;
    v_avisos := v_avisos || jsonb_build_object('key','estrella','label','⭐ OPCIÓN ESTRELLA','severity','high','tipo','alarma',
      'detail','Terciario ≥66% + ' || CASE WHEN v_2esc_confirmada THEN '2ª escalera confirmada' ELSE '≥2 escaleras' END || ' → máxima prioridad.');
    IF v_alarma_prot_2esc AND v_cluster <> 'ultra_prime' THEN
      v_cluster := 'ultra_prime'; v_motivo := v_motivo || ' · upgrade ultra_prime';
    END IF;
  END IF;

  -- Bandas del sub-score
  ...
  -- Nota: la estrella NO infla el número; ordenación por es_estrella se hace en frontend.
...
-- 3) Recompute cohort 77 (only SQL, no detectors)
DO $$
DECLARE r record; BEGIN
  FOR r IN SELECT id FROM public.buildings WHERE cartera_demo_seed = true LOOP
    PERFORM public.compute_cluster_score(r.id);
  END LOOP;
END $$;
