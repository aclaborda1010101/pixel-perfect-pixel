...
WITH owner_contacts AS (
 ...
 SELECT a.owner_id,
    ...
    hs_disposition_conectada(hc.hs_call_disposition) AS conectada,
    hs_disposition_conectada(hc.hs_call_disposition) AND COALESCE(hc.hs_call_duration, 0) >= 15000 AS conversacion_sustancial
   FROM attrib a
