CREATE INDEX IF NOT EXISTS idx_building_tasks_active_due
ON public.building_tasks (due_date, user_id)
WHERE status IN ('pending', 'in_progress') AND task_type <> 'simulation_v5';

CREATE INDEX IF NOT EXISTS idx_building_tasks_completed_at
ON public.building_tasks (completed_at DESC, user_id)
WHERE status = 'completed' AND task_type <> 'simulation_v5';
...
AS $$
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'score', COALESCE(b.cluster_score, b.score, 0),
    'score_activo', b.score_activo,
    'score_propietarios', b.score_propietarios,
    'score_total', b.score_total,
    'score_propietarios_breakdown', b.score_propietarios_breakdown,
    'porcentajes_estado', b.porcentajes_estado,
    'es_estrella', COALESCE(b.es_estrella, false),
    'avisos_inteligentes', b.avisos_inteligentes,
    'score_summary', b.score_summary,
    'score_breakdown', b.score_breakdown,
    'confianza_media', b.confianza_media,
    'cartera_demo_seed', COALESCE(b.cartera_demo_seed, false),
    'cluster_asignado', b.cluster_asignado,
    'comercial', b.comercial,
    'iee_estado', b.iee_estado,
    'metadatos', jsonb_build_object(
      'tenemos_la_nota_simple_', b.metadatos->>'tenemos_la_nota_simple_',
      'barrio', b.metadatos->>'barrio',
      'distrito', b.metadatos->>'distrito'
    ),
    'has_analysis', ba.building_id IS NOT NULL,
    'ventanas_fachada_total', ba.ventanas_fachada_total,
    'ventanas_patios_total', ba.ventanas_patios_total,
    'segundas_escaleras', ba.segundas_escaleras,
    'plantas_levantables', ba.plantas_levantables,
    'tiene_azotea_transitable', ba.tiene_azotea_transitable,
    'esquina', ba.esquina,
    'protegido_historicamente', ba.protegido_historicamente,
    'edificio_reformado', ba.edificio_reformado,
    'gestion_profesional', ba.gestion_profesional,
    'prior_count', COALESCE(pc.prior_count, 0),
    'assigned', a.building_id IS NOT NULL
