-- ============================================================================
-- Scoring reliability — VISIBILIDAD de fiabilidad SIN cambiar el número
-- ----------------------------------------------------------------------------
-- DECISIÓN (alineada con la arquitectura "anti whack-a-mole" del equipo y con el
-- proceso "ningún cambio in-score se promociona sin golden-set"):
-- el NÚMERO del score queda IDÉNTICO al de producción actual. Solo se AÑADE
-- metadata de fiabilidad, que es lo que se pide ("fiabilidad de las variables"):
--  1. Indicador de CONFIANZA (0..1) + lista `datos_incompletos` en el breakdown
--     y un aviso, para que un score basado en inputs ausentes/estimados sea VISIBLE.
--  2. Marca como dato incompleto: mala_gestion sin dato, propietarios sin dato
--     (raw=0), m² estimados, zona no clasificada, 2ª escalera sin confirmar.
--  3. "Posible cambio de uso (2ª escalera sin confirmar)" sube a severidad alta:
--     es accionable (confirmación humana) y desbloquea ultra_prime/hospedaje.
-- NO se tocan pesos, bandas, ni el manejo NULL→0 que afecta al número (eso queda
-- como PROPUESTA a validar con golden-set + decisión de viviendas del cliente).
-- La mejora real de exactitud de los 77 viene de REPROCESAR para poblar datos,
-- no de imputar valores aquí.
-- ============================================================================
...
BEGIN
  ...
  -- Meta de fiabilidad (el frontend lo ignora si no lo conoce; clave dedicada).
  v_breakdown := v_breakdown || jsonb_build_object('key','_confianza','label','Confianza del score','valor_raw', v_confianza, 'peso', 0, 'contribucion', 0, 'datos_incompletos', to_jsonb(v_faltantes));
