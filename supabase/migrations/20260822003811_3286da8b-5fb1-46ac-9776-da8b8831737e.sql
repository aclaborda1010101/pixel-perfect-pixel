...
BEGIN
  ...
  LOOP
    ...
    v_ya := v_owner IS NOT NULL AND EXISTS (
      SELECT 1 FROM public.building_owners bo
      WHERE bo.building_id = r.building_id AND bo.owner_id = v_owner);
    ...
    IF v_owner IS NULL THEN
      ...
      VALUES (initcap(lower(btrim(r.nombre))), 'desconocido'::public.owner_role,
  ...
  RETURN jsonb_build_object('pendientes_procesados', v_pendientes, 'altas_nuevas', v_altas,
                            'enlazados_existentes', v_enlazados,
                            'ya_estaban_con_otra_grafia', v_ya_estaban, 'dry_run', p_dry_run);
END;
$function$;
