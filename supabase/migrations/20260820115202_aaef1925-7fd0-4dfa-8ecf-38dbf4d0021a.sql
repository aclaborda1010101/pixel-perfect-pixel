...
WITH raw_owner_finca AS (
  ...
  SELECT bo_1.owner_id, bo_1.building_id,
     CASE
       ...
       WHEN crm.pct IS NOT NULL THEN crm.pct
       ELSE NULL::numeric
     END AS pct_propiedad,
     CASE WHEN COALESCE(f.crm_valido,false) THEN crm.pct ELSE COALESCE(np.pct, hs.pct, crm.pct) END AS pct_para_score,
     CASE
       ...
       WHEN crm.pct IS NOT NULL THEN 'crm_en_revision'
       ELSE 'desconocido'
     END AS pct_origen,
     CASE
     ...
     CASE
       WHEN NOT COALESCE(f.crm_valido,false) AND np.pct IS NULL AND hs.pct IS NULL AND crm.pct IS NULL
            AND (COALESCE(np.all_invalid,false) OR COALESCE(hs.invalido,false) OR COALESCE(crm.invalido,false)) THEN true
       ELSE false
     END AS pct_invalido,
     CASE WHEN COALESCE(f.crm_valido,false) THEN crm.raw_value ELSE COALESCE(np.raw_value, hs.raw_value, crm.raw_value) END AS pct_raw,
