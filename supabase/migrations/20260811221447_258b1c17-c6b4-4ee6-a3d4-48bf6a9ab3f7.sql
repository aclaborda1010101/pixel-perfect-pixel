...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'user_id', ur.user_id, 'full_name', COALESCE(pr.full_name, pr.email, '')
    ) ORDER BY pr.full_name), '[]'::jsonb) INTO v_comerciales
  FROM public.user_roles ur
  LEFT JOIN public.profiles pr ON pr.id = ur.user_id
  WHERE ur.role = 'comercial_zona'::app_role;

  RETURN jsonb_build_object(
    'from', p_from, 'to', p_to, 'generated_at', now(),
    'comerciales', v_comerciales, 'activas', v_activas, 'realizadas', v_realizadas
  );
END;
$$;
