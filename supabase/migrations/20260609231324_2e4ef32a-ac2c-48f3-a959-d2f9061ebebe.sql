...
ALTER TABLE public.building_analysis
  ADD COLUMN IF NOT EXISTS protegido_raw jsonb DEFAULT '[]'::jsonb;

ALTER TABLE public.catastro_authority_cache
  ADD COLUMN IF NOT EXISTS n_subparcelas_residenciales int;

-- 2) Trigram para fuzzy dirección
CREATE EXTENSION IF NOT EXISTS pg_trgm;
CREATE INDEX IF NOT EXISTS idx_madrid_protegidos_direccion_trgm
  ON public.madrid_edificios_protegidos
  USING gin (direccion_norm gin_trgm_ops);

-- 3) app_settings seed (patio_constants)
INSERT INTO public.app_settings (key, value)
VALUES ('patio_constants', '{
  "densidad_ventanas_por_m_perimetro": {
    "pre_1900": 0.45,
    "1900_1939": 0.40,
    "1940_1969": 0.35,
    "post_1970": 0.30
  ...
  "patio_ingles_perimetro_max_m": 12
}'::jsonb)
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now();

-- 4) Sub-zonas seed (heurística inicial del equipo)
INSERT INTO public.madrid_calles_subzona (calle_norm, numero_desde, numero_hasta, barrio, sub_zona, cluster_override, especificidad, notas)
VALUES
  ('serrano', 1, 60, 'recoletos', 'milla_oro_serrano_bajo', 'ultra_prime', 10, 'Milla de Oro tramo bajo'),
  ('serrano', 61, 250, 'recoletos', 'serrano_medio_alto', 'prime_value_add', 5, 'Serrano medio-alto'),
  ('jose ortega y gasset', 1, 50, 'recoletos', 'ortega_gasset_bajo', 'ultra_prime', 10, 'Ortega y Gasset prime'),
  ('velazquez', 1, 80, 'recoletos', 'velazquez_bajo', 'ultra_prime', 10, 'Velázquez prime'),
  ('claudio coello', 1, 100, 'recoletos', 'coello_prime', 'prime_value_add', 10, 'Claudio Coello'),
  ('goya', 1, 60, 'recoletos', 'goya_bajo', 'prime_value_add', 10, 'Goya prime bajo'),
  ('hermosilla', 1, 70, 'recoletos', 'hermosilla_bajo', 'prime_value_add', 10, 'Hermosilla prime'),
  ('manuela malasana', NULL, NULL, 'universidad', 'malasana_core', 'flex_living_core', 5, 'Manuela Malasaña — flex living'),
  ('topete', NULL, NULL, 'bellas vistas', 'topete_distressed', 'outer_distressed', 5, 'Topete — distressed selectivo'),
  ('amparo', NULL, NULL, 'embajadores', 'amparo_distressed', 'outer_distressed', 5, 'Amparo (Lavapiés) distressed'),
  ('gaztambide', NULL, NULL, 'arapiles', 'gaztambide_flex', 'flex_living_core', 5, 'Gaztambide flex living'),
  ('plaza san miguel', NULL, NULL, 'palacio', 'psm_protegido', 'ultra_prime', 10, 'Plaza San Miguel — entorno protegido')
ON CONFLICT DO NOTHING;

-- 5) compute_cluster_score: viviendas robustas + escaleras XML
...
BEGIN
  ...
  -- Viviendas robustas: md vs autoridad catastral. Si md falta o ratio>500, usar auth.
  ...
  -- Escaleras: incluye n_subparcelas_residenciales (XML DNPRC). Mínimo 1.
