CREATE OR REPLACE VIEW public.v_owner_pct_crm AS
 WITH base AS (
         ...
         SELECT b.building_id,
            ...
            b.normalizado,
            b.invalido,
            (g.grupos = '{}'::text[] OR g.grupos && ARRAY['pd','pd_gan','np','np_gan']) AS computa,
            (g.grupos && ARRAY['pd_gan','np_gan']) AS ganancial,
            ('fam'::text = ANY (g.grupos) AND NOT (g.grupos = '{}'::text[] OR g.grupos && ARRAY['pd','pd_gan','np','np_gan'])) AS familiar,
            'usu'::text = ANY (g.grupos) AS tiene_usufructo
           FROM base b
             LEFT JOIN LATERAL (SELECT derecho_grupos(b.derecho_raw) AS grupos) g ON true
        ), paso1 AS (
...
ALTER VIEW public.v_owner_pct_crm SET (security_invoker = on);