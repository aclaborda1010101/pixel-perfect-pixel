...
WITH agg AS (
  ...
  SELECT agg.id, agg.direccion, agg.ciudad, agg.division_horizontal, agg.owners_count, agg.md, agg.m2_rango,
...
SELECT id, direccion, ciudad, division_horizontal,
  ...
  COALESCE(
    NULLIF(md ->> 'metros_cuadrados_viviendas___clonada_'::text, ''::text),
    NULLIF(md ->> 'metros_cuadrados_viviendas'::text, ''::text)
  )::numeric AS m2_viviendas,
  ...
  COALESCE(
    NULLIF(md ->> 'metros_cuadrados_oficina'::text, ''::text),
    NULLIF(md ->> 'metros_cuadrado_oficina'::text, ''::text)
  )::numeric AS m2_oficina,
  ...
  COALESCE(
    NULLIF(md ->> 'valoracion_viviendas___clonada_'::text, ''::text),
    NULLIF(md ->> 'valoracion_viviendas'::text, ''::text)
  )::numeric AS valoracion_viviendas,
  NULLIF(md ->> 'valoracion_locales'::text, ''::text)::numeric AS valoracion_locales,
  COALESCE(
    NULLIF(md ->> 'metros_cuadrados__exactos____clonada_'::text, ''::text),
    NULLIF(md ->> 'metros_cuadrados__exactos_'::text, ''::text)
  )::numeric AS m2_totales_exactos
FROM base;