...
WITH raw_owner_finca AS (
  ...
  SELECT bo_1.owner_id, bo_1.building_id,
     CASE
       ...
       WHEN (CASE WHEN NOT cr.de_contacto THEN cr.pct END) IS NOT NULL THEN (CASE WHEN NOT cr.de_contacto THEN cr.pct END)
       WHEN cr.pct IS NOT NULL THEN cr.pct
       ELSE NULL::numeric
     END AS pct_propiedad,
     CASE WHEN COALESCE(f.crm_valido,false) THEN cr.pct ELSE COALESCE(np.pct, (CASE WHEN NOT cr.de_contacto THEN cr.pct END), cr.pct) END AS pct_para_score,
     CASE
       ...
       WHEN (CASE WHEN NOT cr.de_contacto THEN cr.pct END) IS NOT NULL THEN CASE WHEN b.porcentajes_estado = 'verificado' THEN 'building_owners' ELSE 'en_revision' END
     ...
     CASE
       ...
       WHEN (CASE WHEN NOT cr.de_contacto THEN cr.pct END) IS NOT NULL THEN COALESCE(cr.normalizado,false) AND b.porcentajes_estado = 'verificado'
       ELSE false
     END AS pct_normalizado,
     CASE
       WHEN NOT COALESCE(f.crm_valido,false) AND np.pct IS NULL AND (CASE WHEN NOT cr.de_contacto THEN cr.pct END) IS NULL AND cr.pct IS NULL
            AND (COALESCE(np.all_invalid,false) OR COALESCE(cr.invalido,false) OR COALESCE(cr.invalido,false)) THEN true
       ELSE false
     END AS pct_invalido,
     CASE WHEN COALESCE(f.crm_valido,false) THEN cr.raw_value ELSE COALESCE(np.raw_value, cr.raw_value, cr.raw_value) END AS pct_raw,
     CASE WHEN COALESCE(f.crm_valido,false) THEN cr.pct ELSE nr.pct_pleno END AS pct_pleno,
  ...
  LEFT JOIN public.v_owner_pct_crm cr ON cr.building_id = bo_1.building_id AND cr.owner_id = bo_1.owner_id
...
SELECT o.id AS owner_id,
   ...
   (COALESCE(pr.familiar_crm, false)
    OR (bo.es_influencer AND COALESCE(bs.suma, 0) <= 100.75
