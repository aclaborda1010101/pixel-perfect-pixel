...
BEGIN
  ...
  WHERE v_calle_norm LIKE '%' || calle_norm || '%'
    AND (numero_desde IS NULL OR numero_hasta IS NULL
         OR (v_calle_num IS NOT NULL AND v_calle_num BETWEEN numero_desde AND numero_hasta))
  ORDER BY CASE WHEN numero_desde IS NOT NULL THEN 0 ELSE 1 END,
           especificidad DESC, length(calle_norm) DESC
  LIMIT 1;
  ...
  ELSIF v_viv_md IS NOT NULL AND v_viv_auth IS NOT NULL
        ...
        AND (v_m2_raw / v_viv_md) > 500 AND v_viv_auth <> v_viv_md THEN
  ...
  v_n_escaleras := GREATEST(
    COALESCE(ba.n_escaleras_visor, 0), COALESCE(ba.n_escaleras_en_piso01, 0),
  ...
  IF es_estrella THEN v_score := GREATEST(v_score, 95); END IF;
