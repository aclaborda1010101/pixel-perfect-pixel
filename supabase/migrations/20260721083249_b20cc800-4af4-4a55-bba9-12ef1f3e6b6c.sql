...
BEGIN
  ...
  LOOP
    ...
    v_owner_blocked  := v_txt_owner ~ '(no\s+quiere\s+vender|se\s+niega|nunca\s+(voy a\s+)?vender|no\s+piensa\s+vender|cerrad[oa]\s+a\s+vender)';
    ...
    ELSIF v_owner_blocked AND NOT v_owner_positive AND NOT v_owner_impulsor THEN
  ...
  -- Bloqueadores: con señales positivas dominantes, la penalización es simbólica
