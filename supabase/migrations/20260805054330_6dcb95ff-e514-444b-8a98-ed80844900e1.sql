...
WITH rel AS (
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE e.provider = 'hubspot' AND e.entity_type = 'building'
      AND e.provider_object_type = 'deal' AND e.entity_id = b.id
    ORDER BY e.updated_at DESC NULLS LAST
    LIMIT 1
  ) ext ON true
  LEFT JOIN LATERAL (
    SELECT e2.entity_id FROM external_ids e2
    WHERE e2.provider = 'hubspot' AND e2.entity_type = 'building'
      AND e2.provider_object_type = 'deal' AND e2.provider_id = b.hs_deal_id
    LIMIT 1
  ) rev ON b.hs_deal_id IS NOT NULL AND b.hs_deal_id <> ''
), flags AS (
  SELECT base.*,
    ...
    (base.hs_deal_id IS NOT NULL AND base.hs_deal_id <> '' AND base.deal_ext_provider_id = base.hs_deal_id) AS ck_deal,
    CASE
      WHEN base.hs_deal_id IS NULL OR base.hs_deal_id = '' THEN 'sin_deal'
      WHEN base.deal_ext_provider_id IS NULL THEN 'sin_mapa'
      WHEN base.deal_ext_provider_id <> base.hs_deal_id THEN 'conflicto'
      ELSE 'ok'
    END AS deal_estado,
...
SELECT building_id, direccion, owner_id, nombre, telefono, cuota, cuota_pct,
  ...
  hs_deal_id, deal_ext_provider_id, deal_map_building_id, deal_estado,
  ...
  jsonb_build_array(
    ...
    jsonb_build_object('key','deal','label','Mapeo del negocio de HubSpot del edificio','estado',
      CASE WHEN deal_estado = 'ok' THEN 'PASS' WHEN deal_estado = 'sin_mapa' THEN 'UNKNOWN' ELSE 'FAIL' END,
      'valor', CASE
        WHEN deal_estado = 'sin_deal' THEN 'El edificio no tiene hs_deal_id'
        WHEN deal_estado = 'sin_mapa' THEN 'Sin fila en external_ids para este edificio (hs_deal_id ' || COALESCE(hs_deal_id,'') || ')'
        WHEN deal_estado = 'conflicto' THEN 'external_ids apunta al negocio ' || COALESCE(deal_ext_provider_id,'?')
             || ' y la ficha al ' || COALESCE(hs_deal_id,'?')
             || CASE WHEN deal_map_building_id IS NULL THEN ' (el hs_deal_id no está mapeado a ningún edificio)'
                     WHEN deal_map_building_id <> building_id THEN ' (el hs_deal_id pertenece al edificio ' || deal_map_building_id::text || ')'
                     ELSE '' END
        ELSE 'Negocio ' || COALESCE(hs_deal_id,'') || ' coherente' END,
      'fuente','external_ids(provider=hubspot, entity_type=building, provider_object_type=deal, entity_id=buildings.id).provider_id vs buildings.hs_deal_id'),
  ...
  array_remove(ARRAY[
    ...
    CASE WHEN deal_estado = 'sin_deal' THEN 'Sin negocio de HubSpot en la ficha' END,
    CASE WHEN deal_estado = 'sin_mapa' THEN 'Edificio sin fila de negocio en external_ids' END,
    CASE WHEN deal_estado = 'conflicto' THEN 'Mapeo de negocio incoherente (external_ids ' || COALESCE(deal_ext_provider_id,'?') || ' vs ficha ' || COALESCE(hs_deal_id,'?') || ')' END,
