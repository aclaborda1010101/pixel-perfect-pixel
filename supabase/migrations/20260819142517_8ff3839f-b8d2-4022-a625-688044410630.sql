...
WITH raw_owner_finca AS (
  ...
  SELECT bo_1.owner_id, bo_1.building_id,
    CASE WHEN np.pct IS NOT NULL THEN np.pct
    ...
    CASE WHEN np.pct IS NOT NULL THEN (CASE WHEN b.porcentajes_estado = 'verificado' THEN 'nota_simple' ELSE 'en_revision' END)
    ...
    CASE WHEN np.pct IS NOT NULL THEN (b.porcentajes_estado = 'verificado')
    ...
    CASE WHEN np.pct IS NULL AND chn.building_id IS NULL AND hs.pct IS NULL
    ...
    nr.pct_pleno, nr.pct_nuda, nr.pct_usufructo
