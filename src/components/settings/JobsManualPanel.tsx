...
const JOBS: Job[] = [
  ...
  { key: "call-summary-backfill", label: "Backfill resumen IA HubSpot (hs_call_summary)", fn: "hubspot_backfill_call_summaries", body: { limit: 300, chain: true }, desc: "Trae de HubSpot el resumen IA (hs_call_summary) que ya genera el CRM y lo guarda en hubspot_calls. GRATIS (sin STT). Idempotente. Encadena hasta agotar la cola. Alimenta agent_kpi_checklist y agent_voss_coach como fuente prioritaria.", cost: "bajo" },
  { key: "transcribe-hs", label: "Transcribir llamadas HubSpot (OpenRouter · respaldo)", fn: "transcribe_calls", body: { limit: 15, chain: true }, desc: "RESPALDO. STT OpenRouter sobre hubspot_calls con grabación ≥45s y sin transcripción. Úsalo solo para llamadas que NO tengan hs_call_summary (el resumen IA de HubSpot ya cubre 2.799 gratis). Guarda en hs_call_transcription.", cost: "alto" },
